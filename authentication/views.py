    ...
    if request.method == "POST":
        ...
        if request.POST.get('tname') and request.POST.get('phone') and request.POST.get('address') and request.POST.get('aadharno') and request.POST.get('password'):
            if pass1 != pass2:
                ...
                return render(request, "registration.html", {"tname": request.POST.get('tname'), "phone": request.POST.get("phone"), "email": request.POST.get("email"), "address": request.POST.get("address"), "anumber": request.POST.get("aadharno"), "file": request.POST.get('file'), })
            else:
                try:
    ...
    if request.method == "POST":
        ...
        if request.POST.get('uname') and request.POST.get('phone') and request.POST.get('email') and request.POST.get('password') and request.POST.get('password1'):
            if pass1 != pass2:
                messages.error(
                    request, "your password and confirm password are not same")
                return render(request, "signup_user.html", {"uname": request.POST.get('uname'), "email": request.POST.get('email'), "phone": request.POST.get('phone')})
            else:
                try:
                    saverecord = user()
...
def user_home(request):
    if request.method == "POST":
        ...
        if request.POST.get('form') and request.POST.get('to') and request.POST.get('date'):
            ...
            return render(request, 'searched_list.html', {'date': date, 'form': form, 'to': to, 'schedule': showschedule, 'dt': dt, 'dc': quers})
    ...
    if request.method == "POST":
        if request.POST.get('type') == "General":
            ...
            try:
                ...
                return render(request, "success.html", {"anumber": request.POST.get('anumber'), "amount": request.POST.get("total"), "tseat": abc, "id": request.POST.get('totalseat'), "type": request.POST.get('type'), "date": request.POST.get('date'), "time": request.POST.get('time'), "cname": request.POST.get('cname'), "aname": request.POST.get("agency_name"), "from": request.POST.get("from"), "to": request.POST.get("to"), "busnumber": request.POST.get("busnumber"), "bustype": request.POST.get("bustype")})
...
def download(request, id):
    ...
    return render(request, "download.html", {"from": b_from, "to": bus.b_to, "btime": bus.booking_time, "cname": bus.Customer_name, "cnumber": bus.Customer_number, "aname": bus.Agency_name, "address": agency.address, "anumber": bus.Agency_number, "busnumber": bus.bus_number, "seattype": bus.seat_type, "btype": bus.bus_type, "tid": bus.id, "seats": bus.Seats, "amount": bus.amount, "date": bus.date, "time": bus.time})
